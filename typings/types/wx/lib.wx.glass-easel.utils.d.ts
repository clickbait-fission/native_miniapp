declare namespace WechatMiniprogram.GlassEasel.TypeUtils {
   ...
   */
  type UnionToIntersection<T> = (T extends unknown ? (arg: T) => void : never) extends (args: infer Arg) => void
    ? Arg
    : never;
   ...
   * }
   */
  type SetDataSetter<T, Prefix extends string = never, Count extends number = 4> = Count extends 0
    ...
    : IsAny<T> extends true
      ...
      : UnionToIntersection<
          T extends any[]
            ? {
                [P in keyof T & number]: SetDataSetter<T[P], SetDataStringPath<P, Prefix>, Subtract<Count, 1>> &
   ...
   */
  type ObjectDataPathStrings<T, Prefix extends string = never, Count extends number = 4> = Count extends 0
   ...
   */
  type GetFromDataPath<T, K extends ReadonlyArray<string | number>> = K extends [
    infer F,
  ...
  type Tag<B, T extends symbol> = [IfNeverOrAny<B, unknown>] extends [null | undefined]
    ? B
    : UnTagAll<B> & { readonly [Tag in Tagged]: [...GetTags<B>, T] };

  type UnTag<B, T extends symbol, Tags = GetTagsWithout<B, T>> = Tagged extends keyof IfNeverOrAny<B, unknown>
    ? Tags extends []
      ? UnTagAll<B>
  ...
  type DataWithPropertyValues<TData extends DataList, TProperty extends PropertyList> = TData &
  ...
  type ChainingFilterFunc<TAddedFields extends { [key: string]: any }, TRemovedFields extends string = never> = (
  ...
  type RelationParams = {
    target?: string | Component.TrivialComponentType | Behavior.TrivialInstance | TraitBehavior.Instance<any>;
    type: 'ancestor' | 'descendant' | 'parent' | 'child' | 'parent-common-node' | 'child-common-node';
    linked?: (target: Component.TrivialInstance) => void;
    linkChanged?: (target: Component.TrivialInstance) => void;
    unlinked?: (target: Component.TrivialInstance) => void;
  ...
  > extends ThisType<TMethodCaller> {
    ...
    implement: <TIn extends { [x: string]: any }>(traitBehavior: TraitBehavior.Instance<TIn, any>, impl: TIn) => void;
    ...
    observer<
      ...
      V = GetFromObserverPathString<DataWithPropertyValues<TPrevData, TProperty>, P>,
    >(
      this: void,
      paths: P,
    ...
    observer<
      P extends Array<ObserverDataPathStrings<DataWithPropertyValues<TPrevData, TProperty>>>,
      V = {
        [K in keyof P]: GetFromObserverPathString<DataWithPropertyValues<TPrevData, TProperty>, P[K]>;
      },
    >(
      this: void,
      paths: readonly [...P],
